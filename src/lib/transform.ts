...
import type { Finding, MaskRequest, MaskResponse, TransformDepth, TransformMode, TransformPreview } from "./types";

export function buildTransformPreviews(finding: Finding): TransformPreview[] {
  const transforms: Array<{ mode: TransformMode; depth?: TransformDepth }> = [
...
export function applyTransforms(request: MaskRequest): MaskResponse {
  ...
  const applied: TransformPreview[] = [];
  ...
  return {
    ...
    findings: scanText({ text: transformedText, customRules: request.customRules }).findings
  };
}

function selectTargetFindings(request: MaskRequest): Finding[] {
  ...
  const selectedTypes = new Set(
    request.findings.filter((finding) => requestedIds.has(finding.id)).map((finding) => finding.type)
  );

  return request.findings.filter((finding) => selectedTypes.has(finding.type));
}

function partialMask(finding: Finding): string {
...
function partialDummy(finding: Finding): string {
  if (finding.type === "email") return "user@example.com";
  if (finding.type === "phone") return "010-0000-0000";
  if (finding.type === "ip_address") return "10.0.0.1";
  if (finding.type === "url" || finding.type === "internal_url") return "https://service.example.internal/api/demo";
  if (finding.type === "bearer_token") return "Bearer dummy-token";
  if (finding.type === "api_key" || finding.type === "env_secret") return replaceAssignmentValue(finding, "dummy-api-key");
  return "dummy-token";
}

function fullDummy(finding: Finding): string {
  if (finding.type === "email") return "user@example.com";
  if (finding.type === "phone") return "010-0000-0000";
  if (finding.type === "ip_address") return "192.0.2.10";
  if (finding.type === "url" || finding.type === "internal_url") return "https://example.com/api/demo";
  if (finding.type === "bearer_token") return "Bearer dummy-token";
  if (finding.type === "api_key" || finding.type === "env_secret") return replaceAssignmentValue(finding, "dummy-api-key");
  return "dummy-token";
}

function placeholderFor(finding: Finding): string {
...
function replaceAssignmentValue(finding: Finding, replacement: string): string {
...
function hasValidPatchRange(text: string, finding: Finding): boolean {
