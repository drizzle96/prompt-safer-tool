...
```text
Paste → Scan → Review Findings → Apply / Ignore → Inspect Detection Preview
Custom Rule → Generate with Copilot SDK → Preview → Approve
```
...
2. **Copilot SDK-powered Organization Rule Agent**
   회사마다 다른 티켓 번호, 내부 프로젝트 코드, 배포 URL, 고객사 코드, 운영 환경명 같은 조직별 패턴을 사용자가 직접 정규식으로 작성하지 않아도 더미 예시만으로 추가할 수 있게 한다. Copilot SDK는 이 rule agent의 오케스트레이션 계층으로 동작하며, regex rule 후보, replacement, severity, test cases, false positive/negative guidance를 생성한다. 생성 결과는 deterministic preview API로 검증되고, 사용자가 승인하기 전에는 scan 규칙에 반영되지 않는다.

3. **Azure Functions Safety Boundary**
   Azure Functions는 scan, mask, rule generation/preview, health diagnostics를 담당하는 privacy-preserving serverless boundary다. 원본 프롬프트는 저장하지 않고, raw prompt request body를 로그로 남기지 않으며, health endpoint와 보안 헤더로 배포 증거를 제공한다. Azure OpenAI 또는 Microsoft Foundry는 선택적 provider로만 둔다.
...
| Copilot SDK 필수 | Copilot SDK-powered Organization Rule Agent가 더미 조직 예시를 regex rule, replacement, severity, test cases, false positive/negative guidance로 변환한다. SDK 결과는 preview API로 검증되고 사용자 승인 후 session rule로만 적용된다. |
...
### AI Auto-Grading Evidence

발표 없이 URL, README, PRD, 코드, API 응답을 기준으로 평가될 수 있으므로 다음 증거를 명시적으로 노출한다.

| Evidence | Where it appears |
| --- | --- |
| Copilot SDK is central to the AI workflow | Rule generation response metadata: `generation.engine`, `sdkAttempted`, `runtimeMode`, `fallbackReason`; Rule Builder UI diagnostics; README API examples |
| AI is used outside raw prompt exposure | Rule Agent receives dummy examples or organization policy-like samples, not the original sensitive prompt |
| Generated AI output is verified before use | `/api/rules/preview` validates rule behavior before approval |
| Human approval is required | Generated rules are session-only and inactive until approved |
| Azure is a real runtime layer | `/api/health`, `/api/scan`, `/api/mask`, `/api/rules/generate`, `/api/rules/preview` on Azure Functions |
| Privacy and trust are testable | `cache-control: no-store`, raw prompt logging prohibition, Safe Review `originalText` rejection if retained, Static Web Apps security headers |

### Differentiation From Scheduling-Centric Productivity Apps

Many productivity apps focus on scheduling, summarization, meeting notes, or task planning after a user has already shared work context with an AI system. Safe Prompt Guard focuses on the step before that: making a prompt safe enough to share.

This product treats pre-AI prompt cleanup as a real productivity bottleneck. Developers, PMs, CS agents, and operations users often spend time manually removing emails, tokens, internal URLs, ticket IDs, environment names, and customer-specific values before asking AI for help. Safe Prompt Guard shortens that cleanup step while keeping the user in control.

The AI workflow is also intentionally different. The app does not send raw sensitive prompts to a model for detection. Copilot SDK is used to generate organization-specific detection rules from dummy examples, while deterministic scanning and preview APIs verify those rules before use. This keeps AI useful without making it the first system to see sensitive data.

In one sentence: Safe Prompt Guard is not a scheduling assistant; it is a pre-AI productivity tool that helps people use any AI assistant faster and more safely.
...
10. 제한 시간 안에 Paste → Scan → Review Findings → Apply / Ignore → Inspect Detection Preview가 동작하는 MVP를 완성한다.
...
8. 치환 결과가 원본 textarea를 바꾸지 않고 detection preview에 반영된다.
9. 사용자는 detection preview에서 placeholder, masking, dummy 값이 의도대로 적용되었는지 확인한다.
...
3. Copilot SDK-powered Organization Rule Agent가 regex rule 후보, replacement, severity, 기본 치환 방식, 테스트 케이스, false positive/negative guidance를 생성한다.
...
5. detection preview에서 모든 치환 결과를 확인한다.

### 6.4 Cloud Diagnostics 흐름

1. 자동 채점 또는 제출 전 확인에서 `/api/health`를 호출한다.
2. Azure Functions는 runtime, endpoint list, privacy contract, Copilot configuration 상태를 safe JSON으로 반환한다.
3. 응답에는 secret, token, raw prompt, internal path, stack trace가 포함되지 않는다.
4. `/api/scan`, `/api/mask`, `/api/rules/generate`, `/api/rules/preview`가 배포 URL에서 동작하는지 확인한다.
...
- Safe Review UI와 Copy workflow는 Safe Output 제거 결정에 따라 MVP 필수 범위에서 제외한다.
...
치환 결과는 detection preview에서 먼저 확인하세요.
```
...
| 계층 | 역할 |
| --- | --- |
| Frontend | 입력, 하이라이트, 옵션 선택, detection preview, custom rule 승인 |
| Azure Functions | rule 기반 탐지, 치환, rule generation/preview, optional safe-review compatibility API, health diagnostics |
| Copilot SDK | Organization Rule Agent로 custom rule 후보, 테스트 케이스, false positive/negative guidance 생성 |
...
- Safe Review API를 유지하는 경우에도 원본 `originalText`는 거부하고 치환된 텍스트/metadata만 허용한다.
...
- 적용 전 preview를 제공한다.
- Ignore한 항목은 원본 값이 남는다는 경고를 UI 상태로 표시한다.
- Copilot이 생성한 rule은 검증 없이 자동 적용하지 않는다.
- Rule Agent 결과는 `/api/rules/preview` 검증과 사용자 승인 없이는 적용하지 않는다.
...
- detection preview에서 pending finding이 남아 있으면 경고한다.

```text
아직 처리되지 않은 위험 요소가 있습니다.
AI에 보내기 전에 모두 적용하거나 무시해주세요.
```

### Ignore한 항목

- ignored finding은 원본 prompt와 detection preview에 원본 값이 남을 수 있다.
- UI는 ignored finding이 남아 있음을 명확히 표시한다.
- optional Safe Review API를 유지하는 경우 ignored count와 ignored type metadata만 전달한다.
...
9. detection preview에서 원본 textarea는 유지되고 치환된 값만 preview에 반영되는지 확인한다.
10. Azure 배포 URL에서 `/api/health`, `/api/scan`, `/api/rules/generate`, `/api/rules/preview` 동작을 확인한다.
...
7. Findings Panel
8. Apply / Ignore
9. Detection preview에 applied transform 표시
10. Custom Rule Builder UI
11. `/api/rules/generate`
...
3. Global Fix Bar
4. `/api/health`와 Azure Functions diagnostics
5. Static Web Apps 보안 헤더
6. custom finding을 Apply All에 포함
...
- Apply All이 동작한다.
- Ignore가 동작하고 원본 값이 남는다는 경고를 표시한다.
- 원본 prompt textarea는 유지되고, 적용된 치환 결과는 detection preview에 표시된다.
- `/api/scan`, `/api/mask`, `/api/rules/generate`, `/api/rules/preview`, `/api/health`가 배포 URL에서 동작한다.
- Custom Rule Builder가 메인 화면 또는 주요 액션 영역에서 접근 가능하다.
- Copilot SDK-powered Organization Rule Agent가 더미 예시 기반 rule, test case, false positive/negative guidance를 생성하거나 fallback reason을 명확히 표시한다.
...
- [ ] Apply All 동작 확인
- [ ] Detection preview에 치환 결과가 표시되고 원본 textarea는 유지됨
- [ ] Custom Rule Builder가 메인 기능처럼 노출됨
- [ ] Copilot SDK Rule Builder 데모 가능
- [ ] Rule generation 응답에 `generation.engine`, `sdkAttempted`, `runtimeMode`, `fallbackReason` 진단 표시
